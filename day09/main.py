...
# !Pre-Order Traversal
def preorder_traversal(root: Optional[TreeNode]) -> None:
    if root:
        print(root.data, end=" ")
...
def postorder_traversal(root: Optional[TreeNode]) -> None:
    if root:
...
def level_order_traversal(root: Optional[TreeNode]) -> None:
    if not root:
        return
